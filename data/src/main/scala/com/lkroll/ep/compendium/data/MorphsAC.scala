...
object MorphsAC {
  ...
  val basicPod = MorphModel(
    ...
    morphType = Pod,
    descr =
      "Basic pods are essentially lowercost pod versions of a splicer morph. To keep expenses down, this pod has slightly more mechanical parts than most other pods. As a result, it has more obvious cybernetic components.",
  ...
  val bouncer = MorphModel(
    ...
    enhancements =
      Seq("Basic Biomods", "Basic Mesh Inserts", "Cortical Stack", "Grip Pads", "Oxygen Reserve", "Prehensile Feet"),
  ...
  val cloudSkate = MorphModel(
    ...
    morphType = Biomorph,
    descr =
      "Like the surya and hulder, the cloud skate is another biomorph that pushes the edge of what is possible. It is designed to be able to live in the atmospheres of gas giants, including Jupiter. This morph is shaped roughly like a tailless stingray with a pair of slender arms. The version of the swim bladder used by this morph provides neutral buoyancy in dense atmospheres, preventing it from sinking below a safe depth.",
    enhancements = Seq(
      "Basic Biomods",
      "Basic Mesh Inserts",
      "Bioweave Armor (Light)",
