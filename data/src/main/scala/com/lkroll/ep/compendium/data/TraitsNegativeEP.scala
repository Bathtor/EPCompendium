...
object TraitsNegativeEP {
  ...
  val morphingDisorderLevel1 = EPTrait(
    ...
    applicability = Ego,
    descr =
      "Adapting to new morphs is particularly challenging for this character. The character suffers a –10 modifier per level on Integration Tests (p. 271) and Alienation Tests (p. 272).",
    cp = 10,
    source = Sources.ep,
    sourcePage = 150
  );
