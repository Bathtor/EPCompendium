...
import com.lkroll.ep.compendium.MorphType._
...
object Bioware {
  ...
  val echolocation = Augmentation(
    ...
    category = "Bioware",
    appliesTo = List(Biomorph, Pod),
    descr = "The character possesses sonar similar to that of a bat or dolphin. The character bounces brief ultrasonic pulses off their surroundings and uses them to form an image of these surroundings through the pattern of reflections of these pulses received by the character’s ears. For more details, see Using Enhanced Senses, p. 302. This augmentation works in both air and water and has a range of 20 meters in air and 100 meters in water.",
    price = Cost.Low,
    source = Sources.ep,
    sourcePage = 301
  )
  ...
  val lightBioweaveArmor = Augmentation(
    ...
    category = "Bioware",
    appliesTo = List(Biomorph, Pod),
    descr = "Bioweave armor involves lacing the morph’s skin with artificial spider silk biological fibers. This provides an Armor rating of 2/3 without changing the appearance, texture, or sensitivity of the morph’s skin. This armor is cumulative with worn armor, but not heavy bioweave or carapace.",
    price = Cost.Low,
    source = Sources.ep,
    sourcePage = 302
  )
  ...
  val muscleAugmentation = Augmentation(
    ...
    appliesTo = List(Biomorph, Pod),
    price = Cost.High,
    descr = "The morph’s muscle mass has been enhanced and toned and myofibers strengthened. Apply a +5 modifier to SOM.",
    source = Sources.ep,
    sourcePage = 305
  )
  ...
  val neurachem = Augmentation(
    ...
    appliesTo = List(Biomorph, Pod),
    price = Cost.High,
    descr = "This bioware modification enhances the character’s chemical synapses and juices their neurotransmitters, drastically speeding up neural connections. Neurachem can be mentally activated or triggered by charged emotions. Level 1 neurachem increases the Speed stat by +1, with no side effect. Level 2 raises the Speed stat by +2, but the character suffers a nervous system fatigue hangover for 1 hour after the boost wears off (–20 modifier to all actions). The speed boost lasts for 30 minutes, though it may be triggered again. Level 2 may be used as Level 1 if desired. [Cost Level 2: Expensive]",
    source = Sources.ep,
    sourcePage = 305
  )
  ...
  val sexSwitch = Augmentation(
    ...
    appliesTo = List(Biomorph, Pod),
    price = Cost.Moderate,
    descr = "A complex suite of alterations allows the character to switch their physical sex to male, female, hermaphrodite, or neuter. This change is mentally triggered but takes approximately 1 week to complete.",
    source = Sources.ep,
    sourcePage = 305
  )
  ...
  val skinPocket = Augmentation(
    ...
    appliesTo = List(Biomorph, Pod),
    price = Cost.Trivial,
    descr = "The morph has a pocket within its skin layer, capable of holding and providing concealment (+30) for small items.",
    source = Sources.ep,
    sourcePage = 305
  )
  ...
  val temperatureTolerance = Augmentation(
    ...
    appliesTo = List(Biomorph, Pod),
    price = Cost.Low,
    descr = "The morph’s temperature regulation and circulation are both substantially enhanced allowing the character to survive in temperatures as low as –30 degrees Celsius and as high as 60 degrees Celsius without discomfort or ill effects.",
    source = Sources.ep,
    sourcePage = 305
  )
