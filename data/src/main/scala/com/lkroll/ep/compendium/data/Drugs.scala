...
object Drugs {
  ...
  val bringIt = Substance(
    ...
    duration = 15.mins,
    effects =
      List(FreeForm("taunt WILLx3 on 3 metter round on you or ally within 1m"), FreeForm("-20 on social skill tests")),
    descr =
  ...
  val grin = Substance(
    ...
    descr =
      "Grin is an effective opiate and pain suppressant. Users may ignore the modifiers from 2 wounds (not cumulative with similar effects) and in fact may not even be aware they are injured. Grin users suffer from tunnel vision, however, and so suffer a –10 modifier on Perception Tests.",
    price = Cost.Low,
    source = Sources.ep,
    sourcePage = 319
  );
  ...
  val alpha = Substance(
    ...
    duration = 2.hours,
    effects = List(
      SkillMod(skill = "Intimidation", mod = 20),
      SkillMod(skill = "Persuasion", mod = 20),
      FreeForm("+10 on Networking Tests where attitude is a factor"),
      FreeForm("No sleep 4days/4h"),
      FreeForm("–10 on COG  tests related to memory and coherent or logical thinkin")
    ),
    descr =
  ...
  val hydra = Substance(
    ...
    effects = List.empty,
    descr =
      "Hydra only affects neo-octopi physiology. Specifically, it provides a euphoric sense of detachment to the ego while also boosting the autonomous activity and unconscious reflexiveness of neo-octopi arms, which already tend to act on their own accord. Though it does not (usually) impact the ego’s control over their arms, any arms that are idle simply do their own thing. The gamemaster decides if and when these arms act and what they do (or, for more fun, you can assign an arm to each other player). The arms mostly act reflex- ively—recoiling from danger, grasping on to nearby objects—but sometimes act as if with intelligent direc- tion, striking opponents, snatching forgotten objects, opening doors, etc. The arms use the octomorph’s Initiative and skills. ",
    price = Cost.Low,
  ...
  val kong = Substance(
    ...
    descr =
      "Kong is a potent drug cocktail that boosts the aggressiveness of non-human primates. Uplifts and smart animals under the influence of kong must make a WIL x 3 Test or automatically attack anyone that exhibits threatening behavior. Kong users can ignore the effects of 1 wound, receive a +5 SOM bonus and a +10 bonus on melee attacks, but suffer a −5 INT modifier. Once engaged in violence, the affected primate must make a WIL x 3 Test to not automatically attack the next nearest target. Attempts to control primates under the influence of kong suffer a −30 modifier to their Animal Handling Tests. Once the effects of this drug wear off, the user suffers −5 SOM for an equivalent duration.",
    price = Cost.Low,
    source = Sources.panopticon,
    sourcePage = 151
  );
  ...
  val raptor = Substance(
    ...
    descr =
      "Raptor is a strength-boosting drug that only affects neo-avians. It increases their wing power and speed, as well as boosting the grip of their talons. Apply a +5 SOM bonus and a +20 modifier to Flight Tests to increase movement (like Sprinting, p. 190, EP). ",
    price = Cost.Moderate,
    source = Sources.panopticon,
    sourcePage = 152
  );
  ...
  val staste = Substance(
    ...
    descr =
      "This drug increases an octomorph brain’s ability to process sensory input from the chemoreceptors in their suckers. This gives an octomorph an enhanced ability to “taste” things akin to a bloodhound’s sense of smell, only tactile-based. Apply a +30 modifier to Perception, Investigation, or similar skill tests based on smell.",
    price = Cost.Low,
    source = Sources.panopticon,
    sourcePage = 152
  );
  ...
  val greyRanks = Substance(
    ...
    descr =
      "Grey ranks is a long-term memory suppression drug also available as a narcoalgorithm. It disrupts the user’s memory formation processes without otherwise affecting their mental faculties. The Titanian intelligence services uses it on both field agents and in so-called “zero-retention briefings.”",
    price = Cost.Moderate,
  ...
  val laughingPoliceman = Substance(
    ...
    descr =
      "This drug is a potent euphoriant used by Titanian intelligence for interrogations. It stimulates feelings of trust and well-being in the user, giving their interrogators a +20 bonus on Deception and Persuasion Tests to extract information.",
    price = Cost.Moderate,
    source = Sources.firewall,
    sourcePage = 175
  );
  ...
  val sweetbo = Substance(
    ...
    descr =
      "Sweet Bo is a rather uncommon drug cocktail that boosts the natural libido of non-human primates, especially Bonobos. Primate Uplifts, Hominids/Humans, and smart animals under the influence of Sweet Bo must make a WIL x 3 Test or automatically attempt to have sex with the next best compatible individual, sexual preferences not withstanding. Sweet Bo users receive a +5 SOM bonus (extra stamina, yay!), but suffer a −5 WIL modifier (no self-control). Attempts to control primates under the influence of Sweet Bo suffer a −30 modifier to their Animal Handling Tests. Once the effects of this drug wear off, the user suffers −5 SOM for an equivalent duration.",
    price = Cost.Moderate,
    source = "Homebrew",
    sourcePage = 0
  );
