...
The space marine variant of the security pod was optimized for orbital skirmishes, boarding actions, suppressing asteroid miner revolts, and other space-theater operations.""",
  ...
  val swarmanoid = MorphModel(
    ...
    enhancements = Seq("360-Degree Vision",
                       "Access Jacks",
                       "Basic Mesh Inserts",
                       "Cortical Stack",
                       "Cyberbrain",
  ...
  val skulkerSwarmanoid = MorphModel(
    ...
    morphType = Synthmorph,
    descr =
      "Skulkers are a stealth swarmanoid designed to infiltrate sensitive areas and avoid detection by sensors. The swarmanoid is not a single shell per se, but rather a swarm of hundreds of insect-sized robotic microdrones. Each individual <bug> is capable of crawling, rolling, hopping several meters, or using nanocopter fan blades for airlift. The controlling computer, cortical stack, and sensor systems are distributed throughout the swarm. Though the swarm can <meld> together into a roughly child-sized shape, the swarm is incapable of tackling physical tasks like grabbing, lifting, or holding as a unit. Individual bugs are quite capable of interfacing with electronics. For additional rules on swarmanoids, see p. 311, EP, and p. 211, Transhuman.",
    enhancements = Seq(
      "360-Degree Vision",
      "Access Jacks",
      "Basic Mesh Inserts",
  ...
  val smartSwarmanoid = MorphModel(
    ...
    morphType = Synthmorph,
    descr =
      "The smart swarm is a robust attempt to get more utility out of the swarmanoid morph design. It makes use of larger modular minidrones (rather than microdrones) that can lock together into various shapes, enabling limited physical actions and tool use. The swarmanoid is not a single shell per se, but rather a swarm of hundreds of insect-sized robotic microdrones. Each individual <bug> is capable of crawling, rolling, hopping several meters, or using nanocopter fan blades for airlift. The controlling computer, cortical stack, and sensor systems are distributed throughout the swarm. Though the swarm can <meld> together into a roughly child-sized shape, the swarm is incapable of tackling physical tasks like grabbing, lifting, or holding as a unit. Individual bugs are quite capable of interfacing with electronics. For additional rules on swarmanoids, see p. 311, EP, and p. 211, Transhuman.",
    enhancements = Seq(
      "360-Degree Vision",
      "Access Jacks",
      "Basic Mesh Inserts",
  ...
  val sylph = MorphModel(
    ...
    enhancements =
      Seq("Basic Biomods", "Basic Mesh Inserts", "Clean Metabolism", "Cortical Stack", "Enhanced Pheromones"),
    movement = Seq("Walker 4/20"),
    aptitudeMax = 30,
    durability = 35,
  ...
  val synthtaur = MorphModel(
    ...
    enhancements = Seq(
      ...
      "Cortical Stack",
      "Cyberbrain",
      "Extra Limbs (6 Arms/Legs)",
      "Grip Pads",
      "Mnemonic Augmentation",
      "Pneumatic Limbs (2 Hind Legs)",
      "Prehensile Feet",
      "Shape Adjusting",
      "Telescoping Limbs (4 Lower Arms/Legs)"
    ),
