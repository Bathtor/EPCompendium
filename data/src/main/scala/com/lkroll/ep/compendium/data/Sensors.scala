...
object Sensors {
  ...
  val hyperspectralImagerPassive = Gear(
    ...
    category = c,
    descr =
      "A hyperspectral imager views the electromagnetic spectrum much like enhanced vision bioware. The passive model perceives infrared to ultraviolet frequencies, while the active model incorporates terahertz and radar imaging as well. See the rules for hyperspectral sensors, p. 162.",
    price = Cost.Moderate,
    source = Sources.panopticon,
    sourcePage = 153
  );
  ...
  val hyperspectralMic = Gear(
    ...
    category = c,
    descr =
      "A hyperspectral mic hears the entire audio spectrum, from infrasonic to ultrasonic frequencies, much like enhanced hearing bioware. See the rules for hyperspectral sensors, p. 162.",
    price = Cost.Moderate,
    source = Sources.panopticon,
    sourcePage = 153
  );
  ...
  val idScanner = Gear(
    ...
    category = c,
    descr =
      "This small hand-held device is used to read the nanotat IDs carried by most morphs (p. 279, EP). It scans the nanobots, decodes the data, and compares the information against linked online databases. To truly be effective, this device must have mesh access to ID databases.",
    price = Cost.Low,
    source = Sources.panopticon,
    sourcePage = 153
  );
  ...
  val qdcd = Gear(
    ...
    category = c,
    descr =
      "These fullerene devices acts as both high-resolution display and camera. They can be painted on almost any surface. Because they do not have lenses, they cannot be detected by lens spotter systems. When not displaying images, they are also difficult to spot visually; apply a −20 modifier. They are hyperspectral, sensing from near-infrared to ultraviolet wave-lengths.",
    price = Cost.High,
    source = Sources.panopticon,
    sourcePage = 153
  );
  ...
  val sensorConcealment = Gear(
    ...
    category = c,
    descr =
      "Many sensors are hidden within everyday objects or constructed to look like something different so as not to be an obvious surveillance tool. This modification to a sensor device applies a −30 modifier to Perception Tests made to spot it and identify it as a sensor.",
    price = Cost.Low,
    source = Sources.panopticon,
    sourcePage = 153
  );
