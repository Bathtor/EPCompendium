...
object Nanotechnology {
  ...
  val generalHive = Gear(
    ...
    category = c,
    descr =
      "General hives are capable of producing any conceivable type of nanobot with the right blueprints and/or programming. Even at their smallest size they are not really portable, with a minimum size being cubes 30 centimeters on a side and a volume of 25 liters.",
    price = Cost.Expensive,
    source = Sources.ep,
    sourcePage = 328
  );
  ...
  val egoBridge = Gear(
    ...
    category = c,
    descr =
      "Ego bridges are vat devices used for uploading and downloading minds. see Backups and Uploading, p. 268, and Resleeving, p. 270.",
    price = Cost.Expensive,
    source = Sources.ep,
    sourcePage = 328
  );
  ...
  val engineerSwarm = Gear(
    ...
    category = c,
    descr =
      "Engineer microswarms are used for various construction purposes: erecting walls, digging tunnels, sealing holes, reinforcing foundations, and so on.",
    price = Cost.Moderate,
    source = Sources.ep,
    sourcePage = 329
  );
  ...
  val injectorSwarm = Gear(
    ...
    category = c,
    descr =
      "Injector microswarms are equipped with tiny needles and a drug payload. A biological target affected by an injector swarm suffers 1 point of damage and the effects of the carried drug, chemical, or toxin. ",
    price = Cost.Moderate,
    source = Sources.ep,
    sourcePage = 329
  );
  ...
  val gardenerSwarm = Gear(
    ...
    category = c,
    descr =
      "This microswarm is useful for a number of agricultural purposes: killing weeds, planting seeds, trimming plants, pollinating, and even harvesting small items. It may also be programmed to simply defoliate an area.",
    price = Cost.Moderate,
    source = Sources.ep,
    sourcePage = 329
  );
  ...
  val proteanSwarm = Gear(
    ...
    category = c,
    descr =
      "This nanoswarm is designed to disassemble other materials and objects and to create a single specific, pre-programmed device from the components (much like a specialized nanofabricator). The proteans must be able to scavenge appropriate raw materials (for example, to create a metallic device the nanobots must transform something else made of metal). The construction time takes 1 hour per cost category of the item (1 hour for a Trivial cost item, 2 hours for Low, etc.).",
    price = Cost.High,
    source = Sources.ep,
    sourcePage = 329
  );
  ...
  val nanomask = Gear(
    ...
    category = c,
    descr =
      "This makeup nanobot application decorates the character’s face in changing artistic and aesthetic styles that also happen to deter the capabilities of facial recognition software. It provides a –30 modifier to attempts to identify the subject using facial recognition. Some habitats ban this accessory.",
    price = Cost.Low,
  ...
  val smartPlankton = Gear(
    ...
    category = c,
    descr =
      "Smart plankton is a nanoswarm variant of smart dust (p. 316, EP) designed for underwater use. The plankton nanobots are designed for aquatic movement and can communicate via flashing lights and infrasound. Like smart dust, smart plankton can record video, audio (including infrasound and ultra-sound), water temperature, chemical traces, as well as movement and other data.",
    price = Cost.Moderate,
    source = Sources.panopticon,
    sourcePage = 152
  );
