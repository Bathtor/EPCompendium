...
object SoftwareEP {
  ...
  val arIllusions = Software(
    name = "AR Illusions",
    descr =
      "These databases of AR clips can be used to create realistic illusions in someone’s entoptic display. see Augmented Reality Illusions, EP p. 259.",
    price = Cost.Moderate,
    source = Sources.ep,
    sourcePage = 331
  );
  ...
  val encryption = Software(
    name = "Encryption",
    descr =
      "Crypto software generates key pairs, encrypts messages using public keys, and decrypts with secret keys. See Encryption, EP p. 253.",
    price = Cost.Low,
    source = Sources.ep,
    sourcePage = 331
  );
  ...
  val exploit = Software(
    name = "Exploits",
    descr =
      "Exploits are hacker tools that take advantage of known vulnerabilities in other software. They are required for intrusion attempts (EP p. 254).",
    price = Cost.High,
    source = Sources.ep,
    sourcePage = 331
  );
  ...
  val sniffer = Software(
    name = "Sniffer",
    descr =
      "Sniffer programs collect all of the transmission that pass to, from, or through the device they are running on. See Sniffing, EP p. 252.",
    price = Cost.Moderate,
    source = Sources.ep,
    sourcePage = 331
  );
  ...
  val spoof = Software(
    name = "Spoof",
    descr =
      "Spoof is a hacker tool used to fake commands and transmissions, making them seem as if they came from another source. See Spoofing Authentication, EP p. 255.",
    price = Cost.Moderate,
    source = Sources.ep,
    sourcePage = 331
  );
  ...
  val tacticalNetworks = Software(
    ...
    descr =
      "These programs allow people in the same squad to share tactical data in real-time. See Tactical Networks, EP p. 205.",
    price = Cost.Moderate,
    source = Sources.ep,
    sourcePage = 331
  );
  ...
  val tracking = Software(
    name = "Tracking",
    descr =
      "This software is used to track people by their presence online. See Scanning, Tracking, and Monitoring, EP p. 251.",
    price = Cost.Moderate,
    source = Sources.ep,
    sourcePage = 331
  );
  ...
  val spasm = Software(
    name = "Spasm",
    descr =
      "Spasm programs are design to incapacitate the ego with excruciating pain. Affected targets must immediately make a WIL × 2 Test. If they fail, they immediately convulse, are disabled, and writhe in agony for 1 Action Turns per 10 full points of MoF. If they succeed, they still suffer a –30 modifier to all actions, which reduces at the rate of 10 points per Action Turn. Due to the nature of the delivery, pain tolerance of any sort has no effect.",
    price = Cost.High,
    source = Sources.ep,
    sourcePage = 332
  );
