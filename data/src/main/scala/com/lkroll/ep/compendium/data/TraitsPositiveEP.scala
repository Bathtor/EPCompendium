...
object TraitsPositiveEP {
  ...
  val adaptabilityLevel1 = EPTrait(
    ...
    applicability = Ego,
    descr =
      "Resleeving is a breeze for this character. They adjust to new morphs much more quickly than most other people. Apply a +10 modifier for Integration Tests (p. 271) and Alienation Tests (p. 272).",
    cp = 10,
    source = Sources.ep,
    sourcePage = 145
  );
  ...
  val adaptabilityLevel2 = EPTrait(
    ...
    applicability = Ego,
    descr =
      "Resleeving is a breeze for this character. They adjust to new morphs much more quickly than most other people. Apply a +20 modifier for Integration Tests (p. 271) and Alienation Tests (p. 272).",
    cp = 20,
    source = Sources.ep,
    sourcePage = 145
  );
  ...
  val ambidextrous = EPTrait(
    ...
    applicability = Ego,
    descr =
      "The character can use and manipulate objects equally well with both hands (they do not suffer the off-hand modifier, as noted under Wielding Two or More Weapons, p. 206). If the character has other prehensile limbs (feet, tail, tentacles, etc.), this trait may be applied to a limb other than the hand. This trait may be taken multiple times for multiple limbs.",
    cp = 10,
    source = Sources.ep,
    sourcePage = 145
  );
  ...
  val exceptionalAptEgo = EPTrait(
    ...
    applicability = Ego,
    descr =
      "The character may raise the maximum for a particular chosen aptitude to 40 rather than 30 (p. 122). Note that this trait just raises the maximum, it does not give the character 10 more aptitude points. This trait may only be taken once.",
    cp = 20,
    source = Sources.ep,
    sourcePage = 146
  );
  ...
  val exceptionalAptMorph = EPTrait(
    ...
    applicability = Morph,
    descr =
      "As a morph trait, it raises the morph aptitude maximum (p. 124) for a particular chosen aptitude by 10 (30 for flats, 35 for splicers, 40 for all others). Note that this trait just raises the maximum, it does not give the character 10 more aptitude points. This trait may only be taken once.",
    cp = 20,
    source = Sources.ep,
    sourcePage = 146
  );
  ...
  val improvedImmune1 = EPTrait(
    ...
    applicability = Morph,
    descr =
      "The morph’s immune system is robust and more resistant to diseases, drugs, and toxins—even more than basic biomods. Apply a +10 modifier whenever making a test to resist infection or the effects of a toxin or drug. This trait is only available to biomorphs.",
    cp = 10,
    source = Sources.ep,
    sourcePage = 146
  );
  ...
  val improvedImmune2 = EPTrait(
    ...
    applicability = Morph,
    descr =
      "The morph’s immune system is robust and more resistant to diseases, drugs, and toxins—even more than basic biomods. Apply a +20 modifier whenever making a test to resist infection or the effects of a toxin or drug. This trait is only available to biomorphs.",
    cp = 20,
    source = Sources.ep,
    sourcePage = 146
  );
  ...
  val painTolerance1 = EPTrait(
    ...
    applicability = Both,
    descr =
      "The character has a high threshold for pain tolerance and is better at ignoring the effects of pain on their abilities and concentration. This allows them to ignore the –10 modifier from 1 wound. This trait is only available for biomorphs.",
    cp = 10,
    source = Sources.ep,
    sourcePage = 146
  );
  ...
  val painTolerance2 = EPTrait(
    ...
    applicability = Both,
    descr =
      "The character has a high threshold for pain tolerance and is better at ignoring the effects of pain on their abilities and concentration. This allows them to ignore both the –10 modifiers from 2 wounds. This trait is only available for biomorphs.",
    cp = 20,
    source = Sources.ep,
    sourcePage = 146
  );
  ...
  val strikingLooks1 = EPTrait(
    ...
    applicability = Morph,
    descr =
      "In an age where biosculpting is easy, good looks are both cheap and commonplace. This morph, however, possesses a physical look that can only be described as striking and unusual, but also somehow alluring and fascinating—even the gorgeous and chiseled glitterati take notice. On social skill tests where the character’s beauty may affect the outcome, they receive a +10 modifier. This modifier is ineffective against xenomorphs or those with the infolife or uplift backgrounds. This trait is only available to biomorphs.\nThis modifier may be purchased for uplift morphs, but at half the cost, and it is only effective against characters with that specific uplift background (i.e., neo-avians, neo-hominids, etc.).\nThe one drawback to this trait is that the character is more easily noticed and remembered.",
    cp = 10,
    source = Sources.ep,
    sourcePage = 148
  );
  ...
  val tough1 = EPTrait(
    ...
    applicability = Morph,
    descr =
      "This morph is resilient than others of its type and can take more physical abuse. Increase their Durability by +5. This also increases Wound Threshold by +1.",
    cp = 10,
    source = Sources.ep,
    sourcePage = 148
  );
  ...
  val tough2 = EPTrait(
    ...
    applicability = Morph,
    descr =
      "This morph is resilient than others of its type and can take more physical abuse. Increase their Durability by +10. This also increases Wound Threshold by +2.",
    cp = 20,
    source = Sources.ep,
    sourcePage = 148
  );
  ...
  val tough3 = EPTrait(
    ...
    applicability = Morph,
    descr =
      "This morph is resilient than others of its type and can take more physical abuse. Increase their Durability by +15. This also increases Wound Threshold by +3.",
    cp = 30,
    source = Sources.ep,
    sourcePage = 148
  );
  ...
  val zoosemiotics = EPTrait(
    ...
    applicability = Ego,
    descr =
      "A character with this trait and the Psi trait does not suffer a modifier when using psi sleights on non-sapient or partly sapient animal species.",
    cp = 5,
    source = Sources.ep,
    sourcePage = 148
  );
