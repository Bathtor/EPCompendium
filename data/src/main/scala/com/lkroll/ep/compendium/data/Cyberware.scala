...
object Cyberware {
  ...
  val deadSwitch = Augmentation(
    ...
    appliesTo = List(Biomorph, Pod, Synthmorph),
    descr =
      "This cortical stack (p. 300) accessory is designed to keep the stack from falling into the wrong hands. If the morph is killed, the dead switch wipes and melts the cortical stack completely, so that the ego cannot be recovered. This option is generally only used by covert operatives with recent backups.",
    price = Cost.Low,
    source = Sources.ep,
    sourcePage = 306
  );
  ...
  val reflexBoosters = Augmentation(
    ...
    appliesTo = List(Biomorph, Pod, Synthmorph),
    descr =
      "The morph’s spinal column and nervous system is rewired with superconducting materials, boosting transmission speed. This raises the character’s REF by +10 and improves Speed by +1.",
    price = Cost.Expensive,
    source = Sources.ep,
    sourcePage = 308
  );
...
If the user has the emergency farcaster augmentation installed in this morph, the failsafe triggers it instead of ejecting the stack and activating the neurotoxin/cyberbrain erasure.""",
  ...
  val qecomm = Augmentation(
    ...
    appliesTo = List(Biomorph, Pod, Synthmorph),
    descr =
      "This is an implanted version of the portable QE comm (p. 315, EP) with a low-capacity qubit reservor. Due to the size of the qubit reservoir, it must usually be implanted in the thoracic cavity. Though uncommon, this implant is sometimes used by moles that require a covert method of communication.",
    price = Cost.Expensive,
    source = Sources.firewall,
    sourcePage = 174
  );
  ...
  val truthFilters = Augmentation(
    ...
    appliesTo = List(Biomorph, Pod, Synthmorph),
    descr =
      "Truth filters can only be used with a cyberbrain or mesh inserts. This module scrambles the signals read from subject’s brain in such a way as to make digital deception scanners (p. 176) worthless. A character with this implant will automatically succeed on Deception Tests against such scanners.\nTruth filters offer no protection against analog deception scanners.",
    price = Cost.Trivial,
    source = Sources.firewall,
    sourcePage = 174
  );
  ...
  val gasJetSystem = Augmentation(
    ...
    appliesTo = List(Biomorph, Pod, Synthmorph),
    descr =
      "This cybernetic system implants nozzles in the morph’s chest, back, and limbs, allowing it to maneuver in microgravity using vectored bursts of gas. It either inputs atmospheric gases through implanted intakes or draws gas from an oxygen reserve implant. This mod applies a +10 modifier to Free Fall Tests.",
    price = Cost.Moderate,
