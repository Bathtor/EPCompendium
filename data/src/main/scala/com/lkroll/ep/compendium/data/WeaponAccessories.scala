...
object WeaponAccessories {
  ...
  val gyroMount = WeaponAccessory(
    name = "Gyro Mount",
    descr = "This weapon harness features a gyro-stabilized weapon mount that keeps the weapon steady. Negates all modifiers from movement.",
    price = Cost.Moderate,
    source = Sources.ep, sourcePage = 342);

  val imagingScope = WeaponAccessory(
    name = "Imaging Scope",
    descr = "Imaging scopes attach to the top of the weapon and act like specs (p. 326). Scopes may also bend like a periscope, along a character to point the weapon and target around corners without leaving cover.",
    price = Cost.Low,
    source = Sources.ep, sourcePage = 342);

  val flashSuppressor = WeaponAccessory(
    name = "Flash Suppressor",
    descr = "This device obscures the muzzle flash on firearms, applying a –10 modifier on Perception Tests to locate a firing weapon by its flash.",
    price = Cost.Low,
    source = Sources.ep, sourcePage = 342);
  ...
  val safetySystem = WeaponAccessory(
    name = "Safety System",
    descr = "A biometric (palmprint or voiceprint) or ego ID (p. 279) sensor is embedded in the weapon, disabling it if anyone other than an authorized user attempts to fire it.",
    price = Cost.Low,
    source = Sources.ep, sourcePage = 342);

  val shockSafety = WeaponAccessory(
    name = "Shock Safety",
    descr = "Just like a safety system, except that an unauthorized user is zapped with an electric shock. Treat as a shock baton (p. 335).",
    price = Cost.Moderate,
    source = Sources.ep, sourcePage = 342);

  val silencer = WeaponAccessory(
    name = "Silencer/Sound Suppressor",
    descr = "This barrel-mounted accessory reduces the sound of a firearm’s discharge (they may not be used on railguns). Apply a –10 modifier on hearing-based Perception Tests to hear or locate the gun’s firing.",
    price = Cost.Moderate,
    source = Sources.ep, sourcePage = 342);
  ...
  val smartmag = WeaponAccessory(
    name = "Smart Magazine",
    descr = "A smart magazine allows the character to pick and choose what ammo round will be fired with each shot. This system leaves less room for bullets, however, so reduce the weapon’s ammunition capacity by half (round up).",
    magazineFactor = 0.5f,
    price = Cost.Moderate,
    source = Sources.ep, sourcePage = 342);
  ...
  val extsmartmag = WeaponAccessory(
    ...
    descr = "A smart magazine allows the character to pick and choose what ammo round will be fired with each shot. This system leaves less room for bullets, however, so combined with the extended magazine, the ammo capacity is normal.",
