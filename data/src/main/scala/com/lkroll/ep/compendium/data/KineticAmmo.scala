...
object KineticAmmo {
  ...
  val hollowPoint = Ammo(
    name = "Hollow-Point",
    descr =
      "Hollow-point bullets are designed to deform and widen once they penetrate a target, thus inflicting more damage.",
    appliesTo = List(Kinetic),
    apMod = 2,
    dmgMod = (1, 0),
    price = Cost.Trivial,
  ...
  val jammer = Ammo(
    name = "Jammer",
    descr =
      "Jammers stick to the target and pulse out jamming electromagnetic signals, jamming the target’s wireless communications. If an Opposed Test is called for, these devices have an Interface of 30. See Radio Jamming, p. 262.",
    appliesTo = List(Kinetic),
  ...
  val plastic = Ammo(
    name = "Plastic",
    descr =
      "Plastic ammo is designed to hurt but not wound targets, and is commonly used for crowd control purposes. Note AV doubled!",
    appliesTo = List(Kinetic),
    dmgMod = DamageMod.Half,
    price = Cost.Trivial,
    source = Sources.ep
  ...
  val reactive = Ammo(
    name = "Reactive",
    descr =
      "The casing on these projectiles is made of reactive materials that release a large amount of energy when subjected to a sudden shock or impact – such as striking a target. In other words, they explode or superheat when they hit. ",
    appliesTo = List(Kinetic),
    apMod = -2,
    dmgMod = +2,
    price = Cost.Low,
  ...
  val reactiveAP = Ammo(
    ...
    descr =
      "This is a tungsten-carbide armor-piercing round with a reactive casing, allowing the ammunition to penetrate even further.",
    appliesTo = List(Kinetic),
    apMod = -6,
    dmgMod = -1,
    price = Cost.Moderate,
  ...
  val splash = Ammo(
    name = "Splash",
    descr =
      "Splash rounds carry a payload like capsule ammo, but are designed to break upon impact rather than penetrating, splashing their contents on the target’s exterior. Splash rounds are typically loaded with paint, taggant nanobots, tracker dye, and similar substances.",
    appliesTo = List(Kinetic),
    dmgMod = DamageMod.NoDamage,
    price = Cost.Trivial,
    source = Sources.ep
  ...
  val zap = Ammo(
    name = "Zap",
    descr =
      "Zap rounds are rubber or gel bullets that create an electric charge upon firing in a piezoelectric like manner to stun the target effectively with both the bullet and the electric shock.",
    appliesTo = List(Kinetic),
    apMod = +2,
  ...
  val biter = Ammo(
    name = "Biter",
    descr =
      "Biters are specially designed to fragment in opposite proportion to the hardness of the target they strike. For hard targets (synthmorphs), they frag- ment very little, blasting a big hole. For soft targets (biomorphs), they fragment and tumble in multiple directions within the body.",
    appliesTo = List(Kinetic),
    dmgMod = (1, 0),
    price = Cost.Low,
    source = Sources.ep
  ...
  val flayer = Ammo(
    name = "Flayer",
    descr =
      "Flayers have nanosensors to detect an oncoming impact, shooting out monomolecular barbs as they are about to strike a target. These monowires cut through the target along with the bullet, inflicting additional damage.",
    appliesTo = List(Kinetic),
    dmgMod = +2,
    price = Cost.Low,
    source = Sources.ep
  ...
  val laserGuided = Ammo(
    name = "Laser-Guided",
    descr =
      "These bullets function like homing smart rounds (apply the +10 attack modifier), except rather than requiring a smartlink system, they lock onto the reflection of the laser sight used to paint the target. Laser-guided bullets may also be used for indirect fire (p. 195).",
    appliesTo = List(Kinetic),
    dmgMod = DamageMod.Effect("+10 Attack"),
    price = Cost.Low,
    source = Sources.ep
  ...
  val zero = Ammo(
    name = "Zero",
    descr =
      "Similar to homing smart rounds, zero bullets identify the target when fired via smartlink. Whether the round hits or misses, however, it sends telem- etry data back to the next zero bullet, allowing it to course-correct and “zero in” to hit the target (or hit more accurately). Apply a +10 modifier to each shot (or burst) fired after the first against the same target in the same Action Turn.",
    appliesTo = List(Kinetic),
