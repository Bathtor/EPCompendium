...
object MorphsDF {
  ...
  val daitya = MorphModel(
    ...
    enhancements = Seq(
      ...
      "Pneumatic Limbs (Arms)",
      "Puppet Sock",
      "Radar",
      "4 Weapon Mounts (Disassembly Tools; 2 fixed, 2 articulated)",
      "Wrist-Mounted Tools"
    ),
  ...
  val dragonfly = MorphModel(
    ...
    morphType = Synthmorph,
    descr =
      "The dragonﬂy robotic morph takes the shape of a meter-long ﬂexible shell with multiple wings and manipulator arms. Capable of near-silent turbofan-aided ﬂight in Earth gravity, dragonﬂy bots fare even better in microgravity",
    enhancements =
      Seq("Access Jacks", "Basic Mesh Inserts", "Cortical Stack", "Cyberbrain", "Mnemonic Augmentation", "Wings"),
  ...
  val dvergr = MorphModel(
    ...
    morphType = Biomorph,
    descr =
      "Dvergar (plural of dvergr) are biomorphs designed for comfortable operation in highgravity environments. They feature a reinforced skeletal structure and sturdier muscle masses. They are easily identifed by their slightly squat, thick-necked, tough appearances.",
  ...
  val faust = MorphModel(
    ...
    morphType = Biomorph,
    descr =
      "Developed in secret by a small group of unaffliated async genehackers, this morph is not a publicly known model. It is only available to individuals who discover a connection to this unnamed, underground async network and then download the template from one of several secret mesh sites. The faust is a modifed menton morph. Even under the most thorough scans, it appears to be nothing more than a mildly personalized version of that morph.",
  ...
  val fenrir = MorphModel(
    ...
    enhancements = Seq(
      ...
      "T-Ray Emitter",
      "Weapon Mount (External, Articulated, 8)"
    ),
    traits = Seq("Large Size"),
  ...
  val flyingSquid = MorphModel(
    ...
    enhancements = Seq(
      ...
      "Cortical Stack",
      "Cyberbrain",
      "Extra Limbs (8 arms, 2 tentacles)",
      "Grip Pads",
