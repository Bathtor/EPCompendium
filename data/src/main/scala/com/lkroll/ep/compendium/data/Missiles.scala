...
object Missiles {
  ...
  def template(name: String, descr: String, apMod: APMod = APMod.Id,
               ...
               areaMod: DamageAreaMod = DamageAreaMod.Replace(DamageArea.Blast), price: Cost, source: String): MissileTemplate = (size: MissileSize) => {
    Ammo(
      s"$name ${size.label}",
      descr,
      List(Seeker(size)),
  ...
  val heap = template(
    ...
    descr = "A design only available for seekers (not grenades), HEAP warheads use high explosives to blast a path for a penetrating round. HEAPs lose –4 damage per meter distance from the blast, as opposed to the usual –2.",
    apMod = -8,
    dmgMod = (3, 12),
    typeMod = Kinetic,
    price = Cost.Moderate,
