...
object SprayWeapons {
  ...
  val buzzer = Weapon(
    ...
    descr =
      "Equipped with a specialized nanobot hive, Buzzers are used to spray a nanoswarm (p. 328) on a target or area. They have a limited capacity of swarms, though the nanohive can construct one new swarm each hour. This weapon is two-handed.",
    damage = (0.d10 + 0),
    effect = Some("Nanoswarm"),
    ap = 0,
    price = Cost.Moderate,
    range = Range.Ranged(5, 15, 30, 50),
    gun = GunExtras(modes = Set(SS), magazineSize = 3),
    source = Sources.ep
  );
  ...
  val shardPistol = Weapon(
    ...
    gun = GunExtras(modes = Set(SA, BF, FA), magazineSize = 100),
    source = Sources.ep
  );
  ...
  val shredder = Weapon(
    ...
    `type` = WeaponType.Spray,
    descr =
      "A heavier version of the shard pistol, the shredder fires a larger cloud of lethal flechettes, enough to shred a portion of the target into a fine mist.",
  ...
  val torch = Weapon(
    ...
    price = Cost.Moderate,
    range = Range.Ranged(5, 15, 30, 50),
    gun = GunExtras(modes = Set(SS), magazineSize = 20),
    source = Sources.ep
  );
