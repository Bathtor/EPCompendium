...
object Disorders {
  ...
  val atavism = Disorder(
    ...
    gameEffect =
      "The player and gamemaster should discuss how much of the uplift’s nature is lost and adjust game penalties accordingly. It is important to note that other uplifts view atavistic uplifts with something akin to horror and will usually have nothing to do with them.",
    source = Sources.ep,
    sourcePage = 212
  );
  ...
  val adhd = Disorder(
    ...
    gameEffect =
      "Perception and related skill penalties. Increased difficulty modifiers to task actions, particularly as the action drags on.",
    source = Sources.ep,
    sourcePage = 212
  );
  ...
  val autophagy = Disorder(
    ...
    gameEffect =
      "Anytime an uplifted octopus with this disorder is placed in a stressful situation they must make a successful WIL × 3 Test or begin to consume one of their limbs.",
    source = Sources.ep,
    sourcePage = 212
  );
  ...
  val bipolar = Disorder(
    ...
    gameEffect =
      "Similar to depression, but when manic the character must make a WIL × 3 Test to not do some action that may be potentially risky. They will also try to convince others to go along with the idea.",
    source = Sources.ep,
    sourcePage = 212
  );
  ...
  val bodyDysmorphia = Disorder(
    ...
    gameEffect =
      "Because of the nature of Eclipse Phase and the ability to swap out and modify a body, this is a fairly common disorder. It is sug- gested that characters with this suffer increased or prolonged resleeving penalties since they are unable to fully adjust to the reality of their new morph.",
    source = Sources.ep,
    sourcePage = 212
  );
  ...
  val borderline = Disorder(
    ...
    gameEffect =
      "The character needs to be around others and will not be left alone, however they also are not quite able to relate to others in a normal way and may also take risks or make impulsive decisions.",
    source = Sources.ep,
    sourcePage = 212
  );
  ...
  val depression = Disorder(
    ...
    gameEffect =
      "Depressives often lack the will to take any sort of action, often to the point of requiring a WIL × 3 Test to engage in sustained activity.",
    source = Sources.ep,
    sourcePage = 212
  );
  ...
  val hypochondria = Disorder(
    ...
    gameEffect =
      "A subject that is hypo- chondriac will often behave as though they are under the effects of some other disorder or physical malady. This can be consistent over time or can be different and ever changing. They will react with hostility to claims that they are faking or not actually ill.",
    source = Sources.ep,
    sourcePage = 213
  );
  ...
  val impulseControl = Disorder(
    ...
    gameEffect =
      "Similar to OCD, if the character doesn’t engage in the behavior they will grow increasingly disturbed and suffer penalties to all actions until they are able to engage in the compulsion that alleviates their anxiety.",
    source = Sources.ep,
    sourcePage = 213
  );
  ...
  val insomnia = Disorder(
    ...
    gameEffect =
      "Due to the lack of meaningful sleep, insomniacs should suffer from blanket penalties to perception-related tasks or anything requiring concentration or prolonged fine motor abilities.",
    source = Sources.ep,
    sourcePage = 213
  );
  ...
  val mpd = Disorder(
    ...
    gameEffect =
      "When the player is under the effects of another personality, they should be treated as an NPC. In some rare cases the player and the gamemaster can work out the second personality and allow the player to roleplay this. This does not however constitute an entire new character that can be “turned on” at will.",
    source = Sources.ep,
    sourcePage = 214
  );
  ...
  val ptsd = Disorder(
    ...
    gameEffect =
      "Penalties to task actions, also treat situations similar to the initial episodes that caused the disorder as a phobia.",
    source = Sources.ep,
    sourcePage = 214
  );
  ...
  val alienBehaviouralDisorder = Disorder(
    ...
    gameEffect =
      "Under stress, the character exhibits the symptoms of the impulse control disorder (p. 213, EP), but the behavior on which they feel the need to act is something wholly inhuman. Because some of the urges on which they feel impelled to act are anatomically impossible, the character may seek out unusual augmentations or even self-mutilate in the attempt.",
  ...
  val alienSensoryDisorder = Disorder(
    ...
    gameEffect =
      "If the async rolls a critical failure on the use of a psi sleight or if they take more than 3 DV from a single use of a sleight, there’s a 50% chance that instead of taking the damage, one of their basic 5 senses, determined at random, is temporarily disabled. The sensory organs themselves continue to function; the effect is at the interface between sensory input and the brain. Therefore, implants using that sense are also disabled. For example, if sight is lost, the effect is to the visual cortex of the brain, so the character loses input from cybernetic eyes and the ability to view visual augmented reality input. The virus will re-route around the disabled sense in 1d10 ÷ 2 hours, but during this time, the character may receive disturbing phantom feedback on the new sensory channel.",
    source = Sources.transhuman,
    sourcePage = 155
  );
  ...
  val cosmicAnxietyDisorder = Disorder(
    ...
    gameEffect =
      "When faced with TITAN artifacts or war machines, the character may experience panic attacks where they tremble or have difficulty breathing, leaving them unable to act. Alternately, they may exhibit terror, fleeing from the stimulus.",
    source = Sources.transhuman,
    sourcePage = 155
  );
  ...
  val speciesDysmorphia = Disorder(
    ...
    gameEffect =
      "Very similar to body dysmorphia (p. 212, EP), except that the character’s desire is to emerge as an alien being. The character has difficulty when resleeving (–10 to –30 on Alienation Tests if the morphs is not alien enough) and may seek out exotic body modifications in a quest to become more “alien.”",
    source = Sources.transhuman,
    sourcePage = 155
  );
