...
object Firewall {
  ...
  val exalt = multiImage(
    source = "IMG/Exalt2.jpg",
    caption = "An Exalt Morph holding a Plasma Blade.")(
    MorphsDF.exalt, Blades.plasmaBlade);

  val flat = Image(
    key = MorphsDF.flat,
    source = "IMG/Flat2.jpg".toCPSource,
    metadata = metaWithCaption("A Flat Morph."));

  val fury = Image(
    key = MorphsDF.fury,
    source = "IMG/Fury2.jpg".toCPSource,
    metadata = metaWithCaption("A Fury Morph."));

  val menton = Image(
    key = MorphsMN.menton,
    source = "IMG/Menton2.jpg".toCPSource,
  ...
  val remade = Image(
    key = MorphsOR.remade,
    source = "IMG/Remade2.jpg".toCPSource,
    metadata = metaWithCaption("A Remade Morph."));

  val remade2 = Image(
    key = MorphsOR.remade,
    source = "IMG/Remade3.jpg".toCPSource,
    metadata = metaWithCaption("A Remade Morph."));

  val ruster = Image(
    key = MorphsOR.ruster,
    source = "IMG/Ruster3.jpg".toCPSource,
  ...
  val synth = Image(
    key = MorphsS.synth,
    source = "IMG/Synth2.jpg".toCPSource,
    metadata = metaWithCaption("A Synth Morph."));

  val takko = Image(
    key = MorphsTX.takko,
    source = "IMG/Takko2.jpg".toCPSource,
    metadata = metaWithCaption("A Takko Morph."));

  val torch = Image(
    key = SprayWeapons.torch,
